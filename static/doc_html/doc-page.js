...
(() => {
  ...
  const stylesheet = `
    :host {
      ...
      /* No font-family here on purpose: :host is the nearest ancestor for
         every light-DOM child (slotted or not), so setting one here would
         win over the consuming document's own body{font-family} rule for
         anything that doesn't set its own explicit override — which is
         most of a typical document. Inherit from whatever page uses this. */
      --doc-page-w: 8.5in;
      --doc-page-h: 11in;
    ...
    /* table-layout: fixed — the actual cause of a much-reported "no right
     * margin" bug: with the default 'auto' layout, a single-cell table's
     * 'width:100%' is only ever a hint. If ANYTHING inside the body cell
     * has an intrinsic (min-content) width wider than that 100% — one
     * over-eager flex/grid child among many is enough — the browser grows
     * the whole table past it rather than clipping, and since nothing
     * clips that overflow, the table (and everything in it) silently
     * bleeds past the sheet's own right padding, right up to the true
     * page edge, while the LEFT edge still looks perfectly margined
     * (overflow only ever grows a box rightward/downward from its normal
     * start, never left). 'fixed' makes the single column strictly honor
     * 100% no matter what's inside it, so it's the content's own job (via
     * min-width:0 / minmax(0,...), already used throughout) to shrink
     * and wrap into that — never the table's job to silently grow for it. */
    ...
    @media print {
       ...
       * The header/footer's own inset from the true page edge (their
       * padding a few lines down) is the full page-margin, same as the
       * left/right sides — so a logo/title in the header starts exactly
       * as far from the top edge as the body text starts from the left
       * edge, instead of visibly closer (it used to be page-margin * 0.45,
       * under half the side inset, which read as a cramped/inconsistent
       * top-and-bottom frame against the full-margin left/right one). The
       * spacer height then simply reserves that same inset again on top of
       * the header/footer's own measured content height, so body content
      ...
      ::slotted([slot="footer"]) {
        ...
        padding: 0 var(--doc-page-margin) var(--doc-page-margin);
      }
    }
  `;

...
# Cloud-first document storage — per explicit request: "now we have
# cloudflare connected so this all can be combined in 1 and just make
# folder for Each User in Document folder that will create all the
# subfolder for it with necessary right naming." No more manually
# configuring 8 separate local folder paths in Settings — every document
# type resolves HERE, auto-created, under this install's own data folder.
# It is deliberately NOT user-facing/configurable — an implementation
# detail, not a setting. R2 (photo_store.upload_document(), already wired
# since v1.1.13) is the real source of truth that follows a document
# across every PC; this local folder is kept in sync with it
# (photo_store.sync_down_documents(), called at startup and on a
# background timer further down) purely so everything ALREADY built on
# real local files keeps working completely unchanged: All Docs' own
# folder scan (engine.scan_all), Submissions' submittal PDF merge
# (needs real files on disk — PyMuPDF has no concept of a cloud object),
# "previous documents for this company" continuation lookups
# (engine.previous_for_company), and sidecar JSON files. Rewriting each
# of those to talk to R2 directly would be a much larger, much riskier
# change for the same end result the user actually asked for: no more
# manual folder setup, and every PC sees every document.
CLOUD_DOCS_BASE = os.path.join(engine.DATA_BASE, "cloud_documents")

def folder_for(doc_type, brand=None):
    dtype = (doc_type or "").upper()
    if dtype not in FOLDER_KEYS:
        return ""
    brand = (brand or current_brand() or engine.DEFAULT_BRAND).upper()
    folder = os.path.join(CLOUD_DOCS_BASE, brand, dtype)
    os.makedirs(folder, exist_ok=True)
    return folder

def scanned_do_folder(brand=None):
    """Same auto-managed scheme as folder_for(), for the one non-generated
    document type: a physically scanned, signed Delivery Order. Separate
    from FOLDER_KEYS since it's not a doc_type the FILLERS/RENDERERS
    dicts know about — it never goes through /api/generate at all, only
    the scanner (scanner.py) and the manual browse-for-a-scan flow."""
    brand = (brand or current_brand() or engine.DEFAULT_BRAND).upper()
    folder = os.path.join(CLOUD_DOCS_BASE, brand, "SCANNED_DO")
    os.makedirs(folder, exist_ok=True)
    return folder

# Who generated/scanned each document, and who has since downloaded a
# local copy of it — per explicit request: "somewhere it will be
# mentioned that its created or imported or generated by which user" and
# "if the files was downloaded that will show in the all docs clearly
# visible that it was downloaded with what user, when". A plain JSON
# file next to the document itself (same directory, same stem, .meta.json
# extension) rather than extending engine.py's existing sidecar
# mechanism — that one only exists for HTML_DOC_TYPES and means
# something specific (the data used to regenerate/re-edit the document);
# this is unrelated bookkeeping that applies uniformly to EVERY document
# type, including xlsx-based ones and scanned DOs, which have no sidecar
# at all today. Uploaded to R2 right alongside the document itself (see
# _upload_doc_meta below) so "who generated this" and the download
# history are visible on every PC, not just the one that did either.
def _doc_meta_path(doc_path):
    return os.path.splitext(doc_path)[0] + ".meta.json"

def _read_doc_meta(doc_path):
    try:
        with open(_doc_meta_path(doc_path), "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}

def _write_doc_meta(doc_path, meta):
    try:
        with open(_doc_meta_path(doc_path), "w", encoding="utf-8") as f:
            json.dump(meta, f, indent=2, ensure_ascii=False)
    except Exception:
        pass

def _upload_doc_meta(doc_path, key_suffix):
...
def all_doc_folders(brand=None):
    ...
    included too, regardless of which brand this call is for —
    /api/generate hardcodes gen_brand=SOLOLUCE for every CAT doc (see its
    own comment), so a CAT file always lives under SOLOLUCE's own CAT
    folder no matter which brand is active when the user later opens All
    Docs. Without this, All Docs would silently stop scanning that folder
    (and so stop showing any Sololuce Datasheet at all) the moment the
    active brand drifts away from SOLOLUCE."""
    brand = (brand or current_brand() or engine.DEFAULT_BRAND).upper()
    out = []
    for dtype in FOLDER_KEYS:
        f = folder_for(dtype, brand=brand)
        if f and f not in out:
            out.append(f)
    scanned = scanned_do_folder(brand)
    if scanned and scanned not in out:
        out.append(scanned)
    sololuce_cat = folder_for("CAT", brand="SOLOLUCE")
    if sololuce_cat and sololuce_cat not in out:
        out.append(sololuce_cat)
    return out
...
def api_cat_next_page():
    ...
    cfg = load_cfg()
    folder = folder_for("CAT", brand="SOLOLUCE")
    best = 0
    ...
    for folder in folders:
        ...
        for r in engine.scan_all(folder):
            ...
            if r.get("type", "").upper() in engine.HTML_DOC_TYPES and r.get("ext") == "pdf":
                ...
                if sc.get("status"):
                    r["status"] = sc["status"]
            # Who generated/scanned this, and its download history — see
            # _read_doc_meta()'s own comment. Applies uniformly to every
            # doc type (not gated to HTML_DOC_TYPES like the sidecar
            # block above), including xlsx-based ones and scanned DOs.
            meta = _read_doc_meta(r["path"])
            if meta.get("generated_by"):
                r["generated_by"] = meta["generated_by"]
                r["generated_at"] = meta.get("generated_at")
            if meta.get("downloads"):
                r["downloads"] = meta["downloads"]
            r.pop("path", None)
            recs.append(r)
...
def api_generate():
    ...
    # folder_for() now auto-creates its target — no more "set your folder
    # first" gate here at all (see folder_for()'s own comment on the
    # cloud-first storage model this replaced).
...
def api_scanner_finalize():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": str(e)}), 502
    # Cloud backup, same as every generated document (see /api/generate's
    # own comment) — a scanned, signed DO should show up on every PC too,
    # not just the one it was physically scanned on.
    _scan_by = session.get("user", "")
    def _backup_scan_to_cloud(scan_path=path, scan_brand=brand, scan_filename=filename, scanned_by=_scan_by):
        meta = {"generated_by": scanned_by, "generated_at": datetime.datetime.now().isoformat(), "downloads": []}
        _write_doc_meta(scan_path, meta)
        photo_store.upload_document(scan_path, f"{scan_brand}/SCANNED_DO/{scan_filename}")
        _upload_doc_meta(scan_path, f"{scan_brand}/SCANNED_DO/{scan_filename}")
    threading.Thread(target=_backup_scan_to_cloud, daemon=True).start()
...
def api_doc_download():
    """The "Download" action in All Docs — per explicit request: cloud is
    the default home for every document now (see folder_for()'s own
    comment), and this is the ONLY way a copy lands somewhere the user
    can see/keep outside the app, one file at a time, on this one PC.
    Records who downloaded it and when — an audit trail visible to
    every PC (see _read_doc_meta()/_upload_doc_meta()'s own comment),
    not just this one, since the metadata gets re-uploaded right after."""
    data = request.json or {}
    rel = data.get("rel", "")
    ...
    try:
        ...
        def job(root):
            ...
            p = filedialog.asksaveasfilename(
                parent=root, title="Save document to…",
                initialfile=os.path.basename(path),
                defaultextension=os.path.splitext(path)[1],
            )
            root.attributes("-topmost", False)
            return p
        dest = _run_on_tk_thread(job)
    except Exception as e:
        return jsonify({"ok": False, "error": f"Could not open dialog: {e}"}), 200
    if not dest:
    ...
    downloads.append({"username": username, "at": datetime.datetime.now().isoformat()})
    _write_doc_meta(path, meta)
    # Re-upload the meta (not the document itself — that hasn't changed)
    # so every other PC sees this download entry too. key_suffix mirrors
    # the same brand/doctype/filename shape used everywhere else — folder
    # is one of all_doc_folders()'s own CLOUD_DOCS_BASE/<brand>/<dtype>
    # paths, so its last two components ARE brand and doctype.
    _folder_parts = os.path.normpath(folder).split(os.sep)
    brand_part, dtype_part = (_folder_parts[-2], _folder_parts[-1]) if len(_folder_parts) >= 2 else ("", "")
    if brand_part and dtype_part:
  ...
  <div id=v-settings class=hide style="padding:24px;max-width:640px;margin:0 auto">
   ...
   <div id=settings-main-panel>
    ...
    <!-- The 8 separate local-folder fields this card used to have (one
         per doc type, plus Scanned Delivery Orders) are gone — per
         explicit request, now that Cloudflare is connected: "this all
         can be combined in 1 and just make folder for Each User in
         Document folder that will create all the subfolder for it with
         necessary right naming." Every document now saves straight to
         the shared cloud (see app.py's folder_for()/CLOUD_DOCS_BASE) —
         nothing to point at a local folder for anymore. A document only
         ever lands somewhere visible on disk when explicitly pulled down
         via the Download button in All Docs, which is also where its
         "generated/scanned by" and download history show up. -->
    <div class=card><div class=ch>Document storage</div><div class=cb>
      <p class=muted style="font-size:12px;margin:0">Every document (Quotations, Tax Invoices, Delivery Orders, Proforma Invoices, Payment Receipts, Credit Notes, Expense Reports, Sololuce Datasheets, scanned Delivery Orders) now saves straight to the shared cloud automatically — no folder to set up. Every install sees the same documents in All Docs, tagged with who generated or scanned each one. Use the Download button in All Docs whenever you need an actual copy on this PC.</p>
    </div></div>
...
async function loadCfg(){await fetch('/api/config').then(r=>r.json());
  // Used to be gated on "does this doc type have a folder configured" —
  // every document type now always has a real (auto-managed cloud)
  // folder, see folder_for()'s own comment, so there's no more "not set
  // up yet" state to check for here at all.
  nextNumber();onCompany()}
...
async function downloadDocFromAllDocs(rel){
  toast('Choose where to save…');
  const r=await fetch('/api/doc-download',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({rel})}).then(r=>r.json()).catch(e=>({ok:false,error:e.message}));
  if(r.cancelled)return;
  if(!r.ok){toast('Download failed: '+(r.error||'unknown error'));return}
  toast('Saved.');
...
function renderList(){
  ...
  rows.forEach(r=>{
    ...
    // Download — per explicit request: cloud is the default home for
    // every document now (see folder_for()'s own comment in app.py), so
    // this is the one deliberate way a copy lands somewhere visible on
    // this PC. Records who/when (see /api/doc-download) — the small
    // badge below surfaces that same history right in the row.
    actions+='<button class=rbtn onclick="event.stopPropagation();downloadDocFromAllDocs(\''+rel+'\')" title="Download a copy to this PC">⬇ Download</button>';
    ...
    // "Generated by X" / download history — per explicit request: "it
    // will be mentioned that its created or imported or generated by
    // which user" and "if the files was downloaded that will show...
    // clearly visible... what user, when". Title attribute carries the
    // full download history (could be several entries); the visible
    // badge itself stays compact — just who made it, and a small
    // download-count indicator when there's history to show.
    let attrib='';
    if(r.generated_by){
      const downloads=r.downloads||[];
      const dlTitle=downloads.length
        ?('Downloaded '+downloads.map(d=>(d.username||'?')+' on '+(d.at||'').slice(0,10)).join('; '))
        :'';
...
# Keep the local document cache (CLOUD_DOCS_BASE, see folder_for()'s own
# comment) caught up with whatever every OTHER PC has generated — an
# initial pull right at startup (so All Docs is correct the first time it
# opens, not just after the first tick), then every 30s. Longer interval
# than accounts (8s) since documents are real file transfers, not one
# small JSON — a slower-but-real cadence here beats hammering R2 with
# full bucket listings every few seconds. Best-effort: sync_down_documents
# already swallows "not configured"/offline on its own.
def _documents_sync_loop():
    while True:
        try:
            photo_store.sync_down_documents(CLOUD_DOCS_BASE)
        except Exception:
            pass
        time.sleep(30)
